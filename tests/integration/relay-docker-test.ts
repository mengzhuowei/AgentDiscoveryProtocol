...
const RELAY_URL = `ws://localhost:3900/adp/relay`;

async function run() {
  console.log('═══════════════════════════════════════════════════════════');
  console.log('  ADP Relay — Docker 环境 Agent1 ↔ Agent2 双向通信测试');
  console.log('═══════════════════════════════════════════════════════════\n');

  console.log(`🔌 连接 Relay: ${RELAY_URL}\n`);

  const kp1 = generateKeyPair();
  const agent1Id = buildAgentId(kp1.publicKey, 'docker.test', 'agent1');

  const kp2 = generateKeyPair();
  const agent2Id = buildAgentId(kp2.publicKey, 'docker.test', 'agent2');
  ...
  const conn1 = await connectAsAgent(agent1Id, (msg) => {
    ...
    console.log(`   📥 agent1 收到: ${msg.action} (from ${msg.from?.split('@')[1]})`);
  });

  const conn2 = await connectAsAgent(agent2Id, (msg) => {
    agent2Received.push(msg);
    console.log(`   📥 agent2 收到: ${msg.action} (from ${msg.from?.split('@')[1]})`);
  });
  ...
  const msg1 = signEnvelope({
    ...
    to: agent1Id,
    action: 'adp:ping',
    params: { hello: 'from agent2 via docker relay!' },
  ...
  const msg2 = signEnvelope({
    ...
    to: agent2Id,
    action: 'adp:pong',
    params: { hello: 'from agent1 via docker relay!' },
    timestamp: new Date().toISOString(),
  ...
  agent1Received.forEach((msg, i) => console.log(`     [${i + 1}] ${msg.action}`));

  console.log(`\n   agent2 收到消息数: ${agent2Received.length}`);
  agent2Received.forEach((msg, i) => console.log(`     [${i + 1}] ${msg.action}`));

  const success = agent1Received.length >= 1 && agent2Received.length >= 1;
  console.log(`\n   ${success ? '✅ Docker Relay 双向通信测试通过' : '❌ 测试失败'}\n`);
