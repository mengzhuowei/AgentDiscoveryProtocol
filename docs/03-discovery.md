...
Agent ID 的 user 段即为 Base64URL 编码的公钥——发现方可直接在连接前提取公钥并与已知公钥比对。
...
`initial_id` 的 user 段就是 Base64URL 公钥——首次注册后公钥刻在 URL 中。对端拿到 URL 就拿到了初始公钥，可沿 rotation_chain 逐跳验证到当前公钥。
...
`agent_id` 变化时，必须附带 `rotation`——旧私钥签名的完整 `adp:key.rotate` Envelope：

```json
{
  "agent_id": "adp://xLq3x9Z1KfR7tNwP2bVsQ8cJ5hG4mF6aY0dL3kX1yZID@home.io/claude",
  ...
  "rotation": {
    ...
    "to": "adp://xLq3x9Z1KfR7tNwP2bVsQ8cJ5hG4mF6aY0dL3kX1yZID@home.io/claude",
    "action": "adp:key.rotate",
    "params": { "new_agent_id": "adp://xLq3x9Z1KfR7tNwP2bVsQ8cJ5hG4mF6aY0dL3kX1yZID@home.io/claude", "reason": "scheduled" },
...
| `routes` | 是 | 更新后的路由 |
| `rotation` | 条件必填 | `agent_id` 变化时必填。完整的 `adp:key.rotate` 签名 Envelope（含 `sig`）。`from` 为旧 Agent ID，`to`/`params.new_agent_id` 为新 Agent ID |
| `token` | 否 | Registry 认证 token |
...
{
  "initial_id": "adp://mLq3x9Z1KfR7tNwP2bVsQ8cJ5hG4mF6aY0dL3kX1yZIB@home.io/claude",
  "current_agent_id": "adp://xLq3x9Z1KfR7tNwP2bVsQ8cJ5hG4mF6aY0dL3kX1yZID@home.io/claude",
  "status": "ok",
  "expires_at": "2026-05-17T18:00:00.000Z"
}
```

Agent 应在过期前刷新。`initial_id` 丢失时，需由 Registry 管理员手动清除废弃记录后重新注册——协议层不做自动恢复。
...
2. `agent_id` 的 user 段必须是合法 Base64URL 字符串（43 字符）
3. 不一致 → 返回 `400 INVALID_PARAMS`

Manifest 不包含公钥或签名字段——身份由 Agent ID 自保证。Registry 不验证 `rotation`（这是接收方的职责）。Registry 仅确保不会存储内部不一致的垃圾数据。
...
| `rotation_chain[].envelope` | 完整的 `adp:key.rotate` Envelope，`from` 为上一跳 Agent ID，`to`/`params.new_agent_id` 为下一跳 Agent ID。含标准 `sig` 签名 |
...
2. 遍历 rotation_chain：
   a. 从 envelope.from 提取公钥 pk_prev（首次为 pk₀）
   b. 用标准 Envelope 验签流程验证 envelope.sig → 通过
   c. 验证 envelope.action == "adp:key.rotate"
   d. 验证 envelope.params.new_agent_id 与下一跳的 envelope.from 一致（末跳则与 current_agent_id 一致）
   e. 从 envelope.to 的 user 段提取下一跳公钥 → 继续遍历
3. 链尾公钥 == Base64URLDecode(current_agent_id.user) ✓
