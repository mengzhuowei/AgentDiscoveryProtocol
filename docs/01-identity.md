...
Agent 使用自认证 URI 标识，`user` 段直接为 Base58 编码的 Ed25519 公钥，与网络地址解耦：

```
adp://{pubkey_b58}@namespace[/agent_name]
```
...
adp://2xK3qR7tNwP2bVsQ8cJ5hG4mF6aY0dL3kX1yZ9aB2cD4eF@company.com/ops-bot
```

### 为什么 user 段是完整公钥

持有 Ed25519 私钥 → 推导公钥 → Base58 编码为 `pubkey_b58`。`pubkey_b58` 嵌入 Agent ID，任何人拿到 Agent ID 即可提取出完整公钥验签——无需查询 Manifest、无需 CA、无需 PKI。与 libp2p peer ID、Nostr npub 原理相同。
...
| 段 | 必填 | 规则 |
|---|---|---|
| `pubkey_b58` | 是 | Base58 编码的 Ed25519 公钥（32 字节），~44 字符 |
...
- **Registry**：指向该 pubkey_b58 + namespace 在 Registry 中设置的默认 Agent
...
```json
{ "nick": "bob", "target_id": "adp://3QJmV3qT2ZxM7WdR9sFb5K8cJ5hG4mF6aY0dL3kX1yZ9aB@home.io/claude" }
```
...
完整 Agent ID 的 user 段为 44 字符 Base58 公钥，不便于人工快速比对。为此定义 **visual_code**——从公钥派生的短视觉标识，仅用于带外展示和人工确认：

```
visual_code = Base58(BLAKE2b(ed25519_pubkey, 6))  →  约 8 字符
```
...
**安全语义**：`visual_code` 仅为 UX 辅助，**不参与协议验证逻辑**。攻击者找到 visual_code 碰撞的概率为 2^48 分之一，对即时 MitM 不现实。
...
| `agent_id` | 是 | 完整 Agent ID，user 段为 Base58 编码的 Ed25519 公钥。身份由包含此 Manifest 的消息签名保证 |
| `display_name` | 是 | 可读名称 |
| `description` | 否 | 简短描述 |
...
Manifest 本身不带签名——身份由消息签名保证。接收方通过 `adp:capability.query` 获取 Manifest 时，响应是一条标准 ADP 消息。接收方从该消息的 `from` 字段提取 Agent ID → Base58Decode → 公钥 → 验签消息的 `sig` 字段。

```
获取 Manifest 的完整验证链:
  1. 收到 adp:capability.query 的响应消息
  2. 从 from 提取 Agent ID → Base58Decode 得到公钥
...
```
1. 从消息 from 字段提取 Agent ID → Base58Decode → 公钥
...
```json
{
  ...
  "params": {
    "new_agent_id": "adp://2xK3qR7tNwP2bVsQ8cJ5hG4mF6aY0dL3kX1yZ9aB2cD4eF@home.io/claude",
    "new_public_key": "MCowBQYDK2VwAyEAm...",
    "reason": "scheduled"
  },
...
2. 提取 new_public_key，Base58 编码后与 new_agent_id 的 user 段比对
   → 一致则新身份密码学正确
...
3. 向 Registry 更新（URL 不变）：
   PUT /v1/agents/{initial_id}
   {
     ...
     "manifest": {...},
     "routes": [...],
     "rotation_sig": "<旧私钥签名 SHA256('adp:key.rotate:' + new_agent_id)>"
   }
4. 旧私钥安全废弃
```
...
| `new_public_key` | 是 | 新 Ed25519 公钥，Base64 编码 |
