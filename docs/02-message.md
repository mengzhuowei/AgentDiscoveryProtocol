...
| `sig` | **是** | Ed25519 签名（Base64URL，无填充），覆盖不含 `sig` 的规范化 Envelope |
| `encoding` | 否 | 有效载荷编码，默认 `"json"`。保留 `"cbor"` 用于未来二进制模式 |
| `expires_at` | 否 | 消息过期时间，ISO 8601 毫秒 UTC。用于应用层时效控制 |
| `trace_id` | 否 | W3C 兼容 trace ID，32 hex 字符。跨 Agent 调用链关联 |
| `span_id` | 否 | 本段 span ID，16 hex 字符 |
...
2. 按 ADP Canonical JSON 规范化算法转为字节序列（详见 [`06-signatures.md`](06-signatures.md)）
3. 用 Ed25519 私钥签名
...
2. 提取 timestamp 字段。偏差超过 300 秒 → 拒绝 INVALID_PARAMS（时钟偏差过大）
3. 删除 sig，规范化剩余字段
4. 从 from 的 user 段 Base64URLDecode 得到公钥
...
- 单个消息不超过 **1 MB**
- `sig` 约 86 字节（64 字节原始签名 Base64URL 编码），忽略不计
...
跨 Agent 调用链可通过两个可选字段关联——与 W3C Trace Context / OpenTelemetry 兼容：

| 字段 | 类型 | 说明 |
|------|------|------|
| `trace_id` | string (32 hex) | 整条调用链的唯一标识 |
| `span_id` | string (16 hex) | 当前 Agent 处理段的标识 |

Gateway 转发消息时：
1. 若入站消息携带 `trace_id`，出站消息应保留相同的 `trace_id`
2. 出站消息生成新的 `span_id`（入站的 `span_id` 即为出站 span 的 parent——与 W3C traceparent 语义一致）
3. 若入站消息无 `trace_id` 且 Gateway 需要追踪，可生成新的 `trace_id` 和 `span_id`

实现方可直接将这些字段注入 OpenTelemetry span context，无需额外映射。省略这些字段时无追踪行为，不影响现有逻辑。
...
2. **时间戳新鲜度**：拒绝 `timestamp` 与当前时间偏差超过 **300 秒**的消息（允许合理时钟偏差，覆盖 IoT 设备睡眠周期）。签名将 `id` 和 `timestamp` 纳入覆盖范围，防止攻击者篡改
